...
type CachingRecommender interface {
	...
	Get(spec *apiv1.PodSpec) (*api_mock.Recommendation, error)
}

type cachingRecommenderImpl struct {
	api   api_mock.RecommenderAPI
	cache *TTLCache
}

// NewCachingRecommender creates CachingRecommender with given cache TTL
func NewCachingRecommender(ttl time.Duration, api api_mock.RecommenderAPI) CachingRecommender {
	ca := NewTTLCache(ttl)
...
func (c *cachingRecommenderImpl) Get(spec *apiv1.PodSpec) (*api_mock.Recommendation, error) {
	cacheKey := getCacheKey(spec)
	if cacheKey != nil {
		cached := c.cache.Get(cacheKey)
		if cached != nil {
			return cached.(*api_mock.Recommendation), nil
		}
	}
