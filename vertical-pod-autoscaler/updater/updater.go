...
import (
	...
	"k8s.io/contrib/vertical-pod-autoscaler/updater/update_priority"
)
...
type updater struct {
	vpaLister        api_mock.VerticalPodAutoscalerLister // wait for VPA api
...
func (u *updater) getPodsForUpdate(pods []*apiv1.Pod, vpa *api_mock.VerticalPodAutoscaler) []*apiv1.Pod {

	priorityCalculator := update_priority.NewUpdatePriorityCalculator(&vpa.Spec.ResourcesPolicy, nil)
...
func NewUpdater(kubeClient kube_client.Interface, cacheTTl time.Duration, minReplicasForEvicition int, evictionToleranceFraction float64) Updater {
	return &updater{
		...
		recommender:      recommender.NewCachingRecommender(cacheTTl, api_mock.NewRecommenderAPI()),
		evictionFactrory: eviction.NewPodsEvictionRestrictionFactory(kubeClient, minReplicasForEvicition, evictionToleranceFraction),
	}
}

func newVpaLister(kubeClient kube_client.Interface) api_mock.VerticalPodAutoscalerLister {
	return api_mock.NewVpaLister(kubeClient)
}
