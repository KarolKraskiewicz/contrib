...
func TestRunOnce(t *testing.T) {
	...
	eviction := &test.EvictionControllerMock{}
	...
	vpaLister := &test.VpaListerMock{}
	podLister := &test.PodListerMock{}
	podLister.On("List").Return(pods, nil)

	vpaObj := test.BuildTestVpaObject(containerName, "1", "3", "100M", "1G", labels)
	vpaLister.On("List").Return([]*api_mock.VerticalPodAutoscaler{vpaObj}, nil).Once()
